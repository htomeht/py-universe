# (C) 1998 Joe Strout, 2004 Gabriel Jagenstedt, 2006 Terry Hancock
#   parser
"""
Parsing of sentences given as command, and command and sentence objects.

This module now has to work very closely with stuff from 'locale' module,
due to complexities in different languages' grammar.
"""
...
#----------------------------------------------------------------------
#    Command -- stores references for various parts of a command
#
class Command:
    """
    Command:
        ...
        iobj is short for indirect object
    ...
    def _sentence(self):
        """
        Return a dictionary with the sentence words identified by part of speech.
        """
        fixed = {'subj':self.subj, 'verb':self.verb, 'dobj':self.dobj, 'advb':self.advb}
        iobjs = self.iobjs
        return fixed.update(self.iobjs)

    def __str__(self):
        sentence = {'subj':str(self.subj), 'verb':str(self.verb), 'dobj':str(self.dobj), 'advb':str(self.advb)}
        for key, val in self.iobj:
            sentence[key] = str(val)

        sentence_strs = {}
        for k,v in self._sentence():
            sentence_strs[k] = str(v)
        
        fixed_pos_str = 'Subj: %(subj)s Verb: %(verb)s Dobj: %(dobj)s Advb: %(advb)s ' % sentence_strs
        iobjs_str     = ''.join(["%s: %s" % (k.capitalize(), sentence_strs[k]) for k in self.iobj.keys()]) 
        
        return '<<%s %s>>' % (fixed_pos_str, iobjs_str)
    ...
    def StuffString(self, pStr, pFor=None):
        ...
        @param pStr: string to print with interpolations from command sentence
        @param pFor: FIXME - I don't know what this is! (TJH)
        """
        def expand_noun(base, noun):
            Base = base.capitalize()
            return {base:        noun(0, pFor),
                    Base:        noun(0, pFor).capitalize(),
                    'the'+Base:  noun(locale.article_the, pFor),
                    'The'+Base:  noun(locale.article_the, pFor).capitalize(),
                    'a'  +Base:  noun(locale.article_a,   pFor),
                    'A'  +Base:  noun(locale.article_a,   pFor).capitalize()}

        expanded_nouns = {}
        for base, noun in self._sentence():
            expanded_nouns.update(expand_noun(base, noun))

        expanded_nouns['time'] = string.split(pub.scheduler.GetTime())[0]
       
        return pStr % expanded_nouns
...
class Parser:
    ...
    def Parser(self, pStr=''):
        """
        Break input up into a list of command objects.
        """
        # Extract quoted phrases
        find_double_quotes = re.compile(r'"(\w+\s*)+"')
        find_single_quotes = re.compile(r"'(\w+\s*)+'")
        double_quotes = find_double_quotes.findall(pStr)
        single_quotes = find_single_quotes.findall(pStr)

        pStr = pStr.lower()

        pStr = find_double_quotes.sub("D", pStr)
        pStr = find_single_quotes.sub("S", pStr)
        pStr = pStr.sub(',', ' , ')     # ',' is a conjunction

        # Tokenize words
        words = pStr.split()

        # Replace the quoted strings:
        d = 0
        s = 0
        for word in words:
            if word='D':
                word = double_quotes[d]
                d += 1
            elif word='S':
                word = single_quotes[s]
                s += 1
        ...
        i = 0
        for word in words:
            if word not in self.conjs:
                clauses[-1].append(word)
            else:
                clauses.append([])

        return [self.ParseClause(clause) for clause in clauses]

    def ParseCommand(self, clause):
        """
        Parse a single clause. Identify parts of speech and attempt
        to construct a logical clause from it.
        """
        if locale.SVO:
            return self._SVO_
        # Should we be aware of word-order, or just recognize known words?
        cmd = Command()
        for word in clause:
            if 

    def _SVO_Command(self, clause):
        """
        Parses imperative sentences for subject-verb-object languages (including English).

        Assumes verb is last, or that first clause is the subject.
        """
        pass

    def _SOV_Command(self, clause):
        """
        Parses imperative sentences for subject-object-verb languages (including Japanese).

        Assumes verb is last.
        """
        pass
...
class Symbol:
    """
    Symbol:
        Base class of components and nouns/things.
